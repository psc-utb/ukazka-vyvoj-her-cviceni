        ...
        */
        private bool Initialize(bool forceReinitialize = false) {
            ...
            */
            ...
            // --- (A) ZJIŠTĚNÍ AKTUÁLNĚ VYBRANÉHO NÁSTROJE (náhrada za "selectedTool") ---
            // Unity 6.2: aktivní editorový nástroj = ToolManager.activeToolType (EditorTools API)
            // - pokud je namespace "UnityEditor.TerrainTools", jde o vestavěný Terrain Tool
            // - pokud je to typ tvého TF toolu, je aktivní on
            var activeToolType = ToolManager.activeToolType; // <- veřejné a podporované v 6.x
            ...
            // TODO: sem dosaď typ svého TF nástroje (pokud ho máš jako EditorTool)
            bool isTerrainFormerTool = activeToolType == currentTool.GetType();

            // (Volitelně) tady můžeš ošetřit exkluzivitu:
            // if (isBuiltInTerrainTool && isTerrainFormerTool) { /* řeš konflikt (log/disable/atd.) */ }

            // --- (B) NAJDENÍ INSPEKTORŮ, KTERÉ EDITUJÍ *TENTO* TERRAIN (bez interních typů) ---
            // Původně se šlo přes interní "UnityEditor.TerrainInspector".
            // Ve 6.x je robustní a bezreflektivní cesta: projdi všechny otevřené Editory a filtruj ty,
            // jejichž target je Terrain tohoto GameObjectu.
            ...
            // --- (C) CACHE BLOKY (beze změny funkčnosti) ---
